...
fn main() {
    ...
    unsafe{
        Camera1.lookat(eye, center, up);
        Camera1.viewport(width / 8, height / 8, width * 3 / 4, height * 3 / 4);
        Camera1.projection(-1.0 / (eye - center).norm() as f32);
    }
    ...
    for i in 0..model.nfaces() {
        ...
        for j in 0..3 {
            unsafe { screen_coords[j] = shader.vertex(&Camera1, &model, i as i32, j as i32); }
        }
